...
class WindMsSqlBuilder extends WindSqlBuilder {
	...
	public function getUpdateSql() {
		$sql = sprintf ( self::SQL_UPDATE.'%s'.self::SQL_SET.'%s%s%s', 
			$this->buildFrom (), 
			$this->buildSet (), 
			$this->buildWhere (), 
			$this->buildOrder ()
	...
	public function getDeleteSql() {
		$sql = sprintf ( self::SQL_DELETE.' '.self::SQL_FROM.'%s%s%s', 
			$this->buildFrom (), 
			$this->buildWhere (), 
			$this->buildOrder ()
		);
	...
	public function getSelectSql() {
		$sql = sprintf ( self::SQL_SELECT.'%s%s'.self::SQL_FROM.'%s%s%s%s%s%s', 
			$this->buildDistinct (), 
			$this->buildField (), 
			$this->buildFROM (), 
			$this->buildJoin (), 
	...
	public function getLastInsertIdSql(){
		return sprintf (self::SQL_SELECT.'%s','@@IDENTITY as insertId');
	}
	...
	public function getAffectedSql($ifquery){
		return sprintf (self::SQL_SELECT.'%s',"@@ROWCOUNT AS affectedRows");
	}
