 ...
 * @version 2011-8-12  xiaoxiao
 */
class WindUrlRewriteRouter extends AbstractWindRouter {
	private $urlPatttern = '';
	private $keyValueSep = '';
	private $separator = '';
	private $suffix = '';
	...
	public function parse() {
		$this->isRewrite() && $this->parseUrl();
		$this->setModule($this->getUrlParamValue('module', $this->getModule()));
	 ...
	 * @return string
	 */
	public function buildUrl($action = '', $controller = '', $params = array()) {
		list($module, $controller, $action) = $this->resolveMvc($action, $controller);
		$m = $this->getConfig('module', 'url-param');
	 ...
	 * @return array
	 */
	private function resolveMvc($action, $controller) {
		list($controller, $module) = WindHelper::resolveController($controller);
		!$module && $module = $this->getConfig('module', 'default-value');
		!$controller && $controller = $this->getConfig('controller', 'default-value');
	...
	private function buildRewriteUrl($params) {
		...
		foreach ($this->patterns as $key => $value) {
			if ('*' == $value[0]) {
				$url = str_replace($value, $this->buildNomalKeys($params), $url);
			} else {
	 ...
	 * @param array $params
	 */
	private function parseNomalKeys($key, $urlParams, &$params) {
		$pos = 0;
	...
	private function getUrlParamValue($type, $defaultValue = '') {
		if ($_param = $this->getConfig($type, 'url-param')) {
			$_defaultValue = $this->getConfig($type, 'default-value', $defaultValue);
			$tmp = $this->getRequest()->getRequest($_param, $defaultValue);
