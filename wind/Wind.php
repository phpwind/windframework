...
class Wind {
	...
	public static function application($appName = 'default', $config = array(), $rootPath = '') {
		...
		if (!isset(self::$_app[$appName])) {
			...
			if ($config) {
				is_string($config) && $config = $factory->getInstance('configParser')->parse(
					$config);
				...
				isset($config['components']) && $factory->loadClassDefinitions(
					$config['components']);
			}
			...
			if (!$application instanceof IWindApplication) throw new WindException(
				'[Wind.application] ' . get_class($application), 
				WindException::ERROR_CLASS_TYPE_ERROR);
			$rootPath = $rootPath ? self::getRealPath($rootPath, false, true) : (!empty(
				$config['root-path']) ? self::getRealPath($config['root-path'], false, true) : dirname(
	...
	public static function getAppName() {
		if (!self::$_currentAppName) throw new WindException('Get appName failed.', 
	...
	public static function import($filePath, $recursivePackage = false) {
		...
		public static function register($path, $alias = '', $includePath = false, $reset = false) {
			...
			if ($includePath) {
				...
				if (set_include_path(
					'.' . PATH_SEPARATOR . implode(PATH_SEPARATOR, self::$_includePaths)) === false) {throw new Exception(
		...
		public static function autoLoad($className, $path = '') {
			$path ||
				 $path = isset(self::$_classes[$className]) ? self::$_classes[$className] : $className;
