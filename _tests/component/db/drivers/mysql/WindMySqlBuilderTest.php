...
class WindMySqlBuilderTest extends BaseTestCase {
	...
	public static function provider() {
		return array (array ('pw_posts', '', '', '', '' ), array ('pw_posts', 'a.uid=pw_posts.authorid', '', '', '' ), array ('pw_posts', 'a.uid=b.authorid', 'b', '', '' ), array ('pw_posts', 'a.uid=b.authorid', 'b', 'subject,pid', '' ), array ('pw_posts', 'a.uid=b.authorid', 'b', 'b.subject,b.pid', '' ), array ('pw_posts', 'a.uid=b.authorid', 'b', array ('subject', 'pid' ), 'phpwind_8' ) );
	}
	
	public static function providerWhere() {
		return array (array ('username = "1"', '', '' ), array (array ('age <= ?', 'uid > ?' ), array (3, 4 ), '' ), array (array ('age = 3', 'uid >4' ), '', '' ), array ('uid < ?', 1, true ), array ('username = ? AND uid > ? ', array ('"suqian"', 2 ), false ) );
	}
	
	public static function providerOrder() {
		return array (array (array ('dateline' => 'desc' ), true ), array ('dateline', true ), array ('age', false ), array (array ('dateline' => true, 'age' => false ), true ) );
	}
	...
	public static function providerData() {
		return array (array (array ('a', 'b', 'c' ) ), array (array (array ('1a', '1b', '1c' ), array ('2a', '2b', '2c' ) ) ),array('username'=>array(1,2,3),'age'=>array(1,2,3)) );
	}
	
	public static function providerSet() {
		return array (array ('username = "1"', '' ), array (array ('age <= ?', 'uid > ?' ), array (3, 4 ) ), array (array ('age = 3', 'uid >4' ), '' ), array ('uid < ?', 1 ), array ('username = ? , uid > ? ', array ('"suqian"', 2 ) ) );
	}
	...
	public function testSelect() {
		$result = $this->WindMySqlBuilder->from ( 'pw_members', 'a' )->from ( 'pw_posts', 'b' )->where ( 'a.uid=b.authorid' )->where ( 'b.tid >= ? and b.pid < ? and uid IN ?', array (2, 1000, array (1, 2, 3, 4, 5, 6, 7, 8 ) ) )->field ( 'username', 'uid', 'b.subject' )->select ()->getAllRow ( MYSQL_ASSOC );
