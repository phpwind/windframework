...
class WindForwardTest extends BaseTestCase {
		...
		if ($immediately){
			try {
				...
				$this->assertEquals(array($action, $args, $isRedirect, true), 
					array($e->getForward()->getAction(), $e->getForward()->getArgs(), $e->getForward()->getIsRedirect(), $e->getForward()->getIsReAction()));
				return;
			}
			...
			$this->WindForward->forwardAction($action, $args, $isRedirect, $immediately);
			$this->assertEquals(array($action, $args, $isRedirect, true), 
					array($this->WindForward->getAction(), $this->WindForward->getArgs(), $this->WindForward->getIsRedirect(), $this->WindForward->getIsReAction()));
		}
	}
	...
	public function testForwardRedirect() {
		try {
			...
			$this->assertEquals(array("index.php", true), array($e->getForward()->getUrl(), $e->getForward()->getIsRedirect()));
			return ;
		}
